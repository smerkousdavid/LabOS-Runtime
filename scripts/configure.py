...
DEVICE_ID_FILE = ROOT / "config" / ".device_id"
...
def get_or_create_device_id() -> str:
    """Return a stable 8-char hex device ID, creating one on first run."""
    if DEVICE_ID_FILE.exists():
        stored = DEVICE_ID_FILE.read_text().strip()
        if stored:
            return stored
...
def generate_env(cfg: dict, secrets: dict, device_id: str = "unknown") -> str:
    rt = cfg.get("runtime", {})
    sp = cfg.get("speech", {})
    ...
    lines = [
        ...
        "",
        "# Device identity (stable across restarts, unique per machine)",
...
def main():
    ...
    env_content = generate_env(cfg, secrets, device_id=device_id)
    env_path = ROOT / ".env"
    ...
    example_content = generate_env(cfg, empty_secrets, device_id="<auto-generated>")
